<template>
  <div class="tomato">
      <div class="showHint" v-on:mousedown="animate" v-on:mouseup="doTime">{{hint[hintIndex]}}</div>
    <div id="countdown">
      <span class="time_span" v-on:mousedown="animate" v-on:mouseup="doTime">{{showMinute}}:{{showSecond}}</span>
      <svg
        :width="size"
        :height="size">
        <circle
      ...
      ></circle>
        <circle
          ...
          v-on:mousedown="animate" v-on:mouseup="doTime"
      ></circle>
      </svg>
      <div class="over_image" v-on:mousedown="animate" v-on:mouseup="doTime"></div>
    </div>
  </div>
</template>
...
<style scoped>
  ...
  .over_image{
    ...
    width: 360px;
    height: 360px;
    background-image: url("../../static/img/tomato_over.png");
  ...
  #countdown .time_span{
    ...
    font-size:95px;
    color:#F36F21;
    -webkit-user-select:none;user-select:none;  /*让span变得不可选取*/
  }
  ...
  .showHint{
    ...
    color: aquamarine;
    margin-bottom: 50px;
    -webkit-user-select:none;user-select:none;  /*变得不可选取*/
  }
  body{
    /*display:flex;*/
